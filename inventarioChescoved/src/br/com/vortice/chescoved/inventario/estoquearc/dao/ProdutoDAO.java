...
public class ProdutoDAO extends DAOAb {
	...
	private static final String SELECT_BASE = "SELECT nome,codigo,local_estoque,valor_custo,valor_venda,quantidade FROM PRODUTO_ARC ";
	...
	public void insert(ProdutoModel produto) throws Exception{
		Connection connection = getDBConnection();
        PreparedStatement stmt = null;
        String sql = "insert into produto_arc(codigo,nome,local_estoque,valor_custo,valor_venda,quantidade) values(?,?,?,?,?,?)";
        try {
        	stmt = connection.prepareStatement(sql);
        	...
        	if(e.getMessage().contains("NOME_PRODUTO_ARC_UNIQUE")){
	...
	public void atualizarQuantidade(ProdutoModel produto) throws Exception{
		Connection connection = getDBConnection();
        PreparedStatement stmt = null;
        String sql = "UPDATE  produto_arc SET quantidade = ? WHERE CODIGO = ?";
        try {
        	stmt = connection.prepareStatement(sql);
	...
	public void update(ProdutoModel produto) throws Exception{
		Connection connection = getDBConnection();
        PreparedStatement stmt = null;
        String sql = "UPDATE  produto_arc SET nome = ?,local_estoque=?,valor_custo=?,valor_venda=? WHERE CODIGO = ?";
        try {
        	stmt = connection.prepareStatement(sql);
